...
class RobotPost(object):
    ...
    ACTIVE_UT = 1           # Active UTool Id (register)
    ...
    def __init__(self, robotpost=None, robotname=None, robot_axes = 6, **kwargs):
        ...
        del self.AXES_TRACK[:]
        self.HAS_TRACK = False #I changed this
        self.PROG = []
        self.PROG_LIST = []
        self.PROG_FILES = []
        self.LOG = ''
        ...
        for i in range(len(self.AXES_TYPE)):
            if self.AXES_TYPE[i] == 'T':
                ...
                #self.HAS_TRACK = True I changed this
    ...
    def ProgFinish(self, progname, new_page = False):
        ...
        #    return
        header = ''
        header = header + ('/PROG  %s' % self.PROG_NAME_CURRENT) + '\t  Process\n' # Use the latest name set at ProgStart
        header = header + '/ATTR' + '\n'
        ...
        header = header + 'STORAGE\t\t= SHADOW ONDEMAND;' + '\n'
        ...
        header = header + '/APPL' + '\n'
        header = header + 'PAINT_PROCESS;' + '\n'
        header = header + '  LAST_CYCLE_TIME\t: UNINIT sec;' + '\n'
        header = header + '  LAST_GUN_ON_TIME\t: UNINIT sec;' + '\n'
        header = header + '  DEFAULT_USER_FRAME\t: 1;' + '\n'
        header = header + '  DEFAULT_TOOL_FRAME\t: 1;' + '\n'
        header = header + '  START_DELAY\t\t: 0;' + '\n'
    ...
    def setGO(self, io_var, io_value):
        """Sets a variable (output) to a given value"""
        if type(io_var) != str:  # set default variable name if io_var is a number
            io_var = 'GO[%s:0]' % str(io_var)        
        #if type(io_value) != str: # set default variable value if io_value is a number            
        #    if io_value >= 0:
        #        io_value = 'ON'
        #    else:
